...
async function dayForecast(table, geo) {
  const response = await fetch(
    `https://api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&current=temperature_2m,rain&hourly=temperature_2m,precipitation_probability,rain&forecast_days=1`,
  );
...
async function weekForecast(table, geo) {
  const response = await fetch(
    `https://api.open-meteo.com/v1/forecast?latitude=${geo.lat}&longitude=${geo.lon}&daily=temperature_2m_max,temperature_2m_min,rain_sum,precipitation_probability_max&forecast_days=10`,
  );
  const weather = await response.json();
  const tbody = document.createElement("TBODY");

...
gnuplot.L = function (x, y) {
  ...
  } else ctx.lineTo(x / 10.0, y / 10.0);
};
gnuplot.M = function (x, y) {
  if (gnuplot.zoomed) {
    var zoom = gnuplot.zoomXY(x / 10.0, y / 10.0);
    ctx.moveTo(zoom.x, zoom.y);
  } else ctx.moveTo(x / 10.0, y / 10.0);
};
gnuplot.R = function (x, y, w, h) {
  ...
  } else ctx.fillRect(x / 10.0, y / 10.0, w / 10.0, h / 10.0);
};
gnuplot.T = function (x, y, fontsize, justify, string) {
  ...
  if (gnuplot.zoomed) {
    ...
    yy = zoom.y;
    if (
      gnuplot.plot_xmin < xx &&
      xx < gnuplot.plot_xmax &&
      gnuplot.plot_ybot > yy &&
      yy > gnuplot.plot_ytop
    )
  ...
  if (justify == "") ctx.drawText("sans", fontsize, xx, yy, string);
    ...
    ctx.drawTextRight("sans", fontsize, xx, yy, string);
  else if (justify == "Center")
    ctx.drawTextCenter("sans", fontsize, xx, yy, string);
};
gnuplot.TR = function (x, y, angle, fontsize, justify, string) {
  ...
  if (justify == "") ctx.drawText("sans", fontsize, 0, 0, string);
    ...
    ctx.drawTextRight("sans", fontsize, 0, 0, string);
  else if (justify == "Center")
    ctx.drawTextCenter("sans", fontsize, 0, 0, string);
  ctx.restore();
};
gnuplot.bp = function (x, y) {
  // begin polygon
  ctx.beginPath();
  gnuplot.M(x, y);
};
gnuplot.cfp = function () {
  // close and fill polygon
  ctx.closePath();
  ctx.fill();
};
gnuplot.cfsp = function () {
  // close and fill polygon with stroke color
  ctx.closePath();
...
gnuplot.Pt = function (N, x, y, w) {
  ...
  switch (N) {
    ...
    case 1:
      var ww = (w * 3) / 4;
      ctx.beginPath();
      ctx.moveTo(xx - ww, yy - ww);
      ctx.lineTo(xx + ww, yy + ww);
      ctx.moveTo(xx + ww, yy - ww);
      ctx.lineTo(xx - ww, yy + ww);
      ctx.stroke();
      break;
    ...
    case 5:
      ctx.beginPath();
      ctx.arc(xx, yy, w / 2, 0, Math.PI * 2, true);
      ctx.stroke();
      break;
    default:
    case 6:
...
gnuplot.ZI = function (image, m, n, x1, y1, x2, y2) {
  if (gnuplot.zoomed) {
    ...
    if (x2raw > zoom.x) sw = m * (1 - (x2raw - zoom.x) / (x2raw - x1raw));
    if (y2raw > zoom.y) sh = n * (1 - (y2raw - zoom.y) / (y2raw - y1raw));
    sw = sw - sx;
    sh = sh - sy;
